...
# =========================
st.set_page_config(
    page_title="Temperature — Global Database of Subnational Climate Indicators",
    layout="wide",
...
def _cities_with_data(df_city: pd.DataFrame, iso3_val: str):
    if df_city.empty or not {"City","Date","Temperature (Mean)"} <= set(df_city.columns):
        return []
...
cities_with_data = _cities_with_data(df_city_temp, iso3)
...
with left:
    ...
    if sel_indicator != current_indicator and sel_indicator == "Precipitation":
        try:
            ...
            if st.query_params.get("page") != "2 Precipitation Dashboard":
...
with right:
    ...
    with st.form(key="temp_options_form", clear_on_submit=False):
        ...
        with fc1:
            pending_ds = st.radio(
                ...
                index=["Historical Observations","Projections (SSP)"].index(st.session_state["temp_active_ds"]),
                horizontal=True,
                key="temp_form_ds"
            )
        with fc2:
            pending_freq = st.radio(
                ...
                index=["Monthly","Seasonal","Annual"].index(st.session_state["temp_active_freq"]),
                horizontal=True,
                key="temp_form_freq"
            )
        with fc3:
            pending_src = st.radio(
                "Data Source",
                ["CDS/CCKP", "CRU", "ERA5"],
                index=["CDS/CCKP","CRU","ERA5"].index(st.session_state["temp_active_src"]),
                horizontal=True,
                key="temp_form_src"
            )
    ...
    if apply_clicked:
        changed = (
            ...
            pending_freq != st.session_state["temp_active_freq"] or
            pending_src  != st.session_state["temp_active_src"]
        )
        if changed:
    ...
    st.markdown(
        f"""<div style="margin-bottom:8px">
                <span style="display:inline-block;padding:2px 8px;border-radius:999px;background:#eef2ff;
                    ...
                    {st.session_state['temp_active_ds']}
                </span>
    ...
    def _country_series(df_country: pd.DataFrame, iso3_val: str) -> pd.DataFrame:
        ...
        if c_iso:
            ...
            src = pd.DataFrame()
        need = {"Date","Temperature (Mean)"}
        if src.empty or not need.issubset(src.columns):
            return pd.DataFrame()
        out = pd.DataFrame({
            "date": pd.to_datetime(src["Date"], errors="coerce"),
            "avg": pd.to_numeric(src["Temperature (Mean)"], errors="coerce"),
        })
        if "Temperature (Variance)" in src.columns:
            out["var"] = pd.to_numeric(src["Temperature (Variance)"], errors="coerce")
        return out.dropna(subset=["date"]).sort_values("date").reset_index(drop=True)

    @st.cache_data(show_spinner=False)
    def _city_series(df_city: pd.DataFrame, iso3_val: str, city_name: str) -> pd.DataFrame:
        if df_city.empty or not {"City","Date","Temperature (Mean)"} <= set(df_city.columns):
            return pd.DataFrame()
        c_iso = _iso3_col(df_city)
        if c_iso:
        ...
        out = pd.DataFrame({
            ...
            "avg": pd.to_numeric(tmp["Temperature (Mean)"], errors="coerce"),
        })
        if "Temperature (Variance)" in tmp.columns:
            out["var"] = pd.to_numeric(tmp["Temperature (Variance)"], errors="coerce")
        return out.dropna(subset=["date"]).sort_values("date").reset_index(drop=True)

    series = _city_series(df_city_temp, iso3, sel_city) if sel_city != "Country (all)" else _country_series(df_country_temp, iso3)
    if series.empty:
    ...
    def chart_with_variance(title: str, s: pd.DataFrame):
        ...
        if "var" in s.columns and s["var"].notna().any():
            ...
            fig.update_layout(yaxis2=dict(overlaying="y", side="right", title="Variance (°C²)"))
    ...
    with g2:
        st.plotly_chart(chart_with_variance("Diurnal Temperature Range (placeholder)", series), use_container_width=True)

    g3, g4 = st.columns(2)
    with g3:
        st.plotly_chart(chart_with_variance("Maximum Temperature (placeholder)", series), use_container_width=True)
    with g4:
        st.plotly_chart(chart_with_variance("Minimum Temperature (placeholder)", series), use_container_width=True)
    ...
    with c1: percentile_chart("Percentile — Average Temperature", series)
    with c2: percentile_chart("Percentile — Maximum Temperature (placeholder)", series)
    with c3: percentile_chart("Percentile — Minimum Temperature (placeholder)", series)
